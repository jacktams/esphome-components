#pragma once

#include "esphome/core/component.h"
#include "esphome/components/ble_client/ble_client.h"
#include "esphome/components/esp32_ble_tracker/esp32_ble_tracker.h"

#ifdef USE_SENSOR
#include "esphome/components/sensor/sensor.h"
#endif
#ifdef USE_TEXT_SENSOR
#include "esphome/components/text_sensor/text_sensor.h"
#endif
#ifdef USE_BINARY_SENSOR
#include "esphome/components/binary_sensor/binary_sensor.h"
#endif
#ifdef USE_BUTTON
#include "esphome/components/button/button.h"
#endif

#include <vector>
#include <queue>
#include <functional>

namespace esphome {
namespace automower_ble {

static const char *const TAG = "automower_ble";

// CRC-8/MAXIM-DOW lookup table
static const uint8_t CRC8_TABLE[256] = {
    ,
};

// Protocol command types
enum class CommandType : uint8_t {
  NONE = 0,
  SETUP_CHANNEL,
  HANDSHAKE,
  ENTER_PIN,
  GET_BATTERY,
  IS_CHARGING,
  GET_STATE,
  GET_ACTIVITY,
  GET_ERROR,
  KEEPALIVE,
  START_TRIGGER,
  PAUSE,
  PARK_UNTIL_NEXT,
  SET_MODE_AUTO,
  SET_OVERRIDE_MOW,
};

// Mower states (from protocol)
enum class MowerState : uint8_t {
  OFF = 0,
  WAIT_FOR_SAFETY_PIN = 1,
  STOPPED = 2,
  FATAL_ERROR = 3,
  PENDING_START = 4,
  PAUSED = 5,
  IN_OPERATION = 6,
  RESTRICTED = 7,
  ERROR = 8,
};

// Mower activities (from protocol)
enum class MowerActivity : uint8_t {
  NONE = 0,
  CHARGING = 1,
  GOING_TO_CHARGE = 2,
  MOWING = 3,
  LEAVING_DOCK = 4,
  PARKED_IN_DOCK = 5,
  STOPPED_IN_GARDEN = 6,
};

// Connection state machine
enum class ConnectionState : uint8_t {
  IDLE = 0,
  CONNECTED,
  PAIRING,
  WAITING_PAIRING,
  SUBSCRIBING,
  SETUP_CHANNEL,
  WAITING_CHANNEL_RESPONSE,
  HANDSHAKE,
  WAITING_HANDSHAKE_RESPONSE,
  AUTH_PIN,
  WAITING_PIN_RESPONSE,
  READY,
  SENDING_COMMAND,
  WAITING_RESPONSE,
  ERROR,
};

class AutomowerBLE;

#ifdef USE_BUTTON
class AutomowerStartButton : public button::Button, public Parented<AutomowerBLE> {
 public:
  void press_action() override;
};

class AutomowerPauseButton : public button::Button, public Parented<AutomowerBLE> {
 public:
  void press_action() override;
};

class AutomowerParkButton : public button::Button, public Parented<AutomowerBLE> {
 public:
  void press_action() override;
};

class AutomowerClearBondButton : public button::Button, public Parented<AutomowerBLE> {
 public:
  void press_action() override;
};
#endif

class AutomowerBLE : public PollingComponent, public ble_client::BLEClientNode {
 public:
  void setup() override;
  void loop() override;
  void update() override;
  void dump_config() override;
  float get_setup_priority() const override { return setup_priority::AFTER_BLUETOOTH; }

  void gattc_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if,
                            esp_ble_gattc_cb_param_t *param) override;
  void gap_event_handler(esp_gap_ble_cb_event_t event,
                          esp_ble_gap_cb_param_t *param) override;

  void set_pin(uint16_t pin) { this->pin_ = pin; }

#ifdef USE_SENSOR
  void set_battery_sensor(sensor::Sensor *sensor) { this->battery_sensor_ = sensor; }
#endif
#ifdef USE_TEXT_SENSOR
  void set_state_sensor(text_sensor::TextSensor *sensor) { this->state_sensor_ = sensor; }
  void set_activity_sensor(text_sensor::TextSensor *sensor) { this->activity_sensor_ = sensor; }
  void set_error_sensor(text_sensor::TextSensor *sensor) { this->error_sensor_ = sensor; }
#endif
#ifdef USE_BINARY_SENSOR
  void set_charging_sensor(binary_sensor::BinarySensor *sensor) { this->charging_sensor_ = sensor; }
  void set_connected_sensor(binary_sensor::BinarySensor *sensor) { this->connected_sensor_ = sensor; }
#endif

  void queue_command(CommandType type);

 protected:
  // CRC calculation
  static uint8_t crc8(const uint8_t *data, uint8_t offset, uint8_t length);
  static uint8_t crc8(const std::vector<uint8_t> &data, uint8_t offset, uint8_t length);

  // Packet builders
  std::vector<uint8_t> build_setup_channel_id_();
  std::vector<uint8_t> build_handshake_();
  std::vector<uint8_t> build_command_(uint16_t major, uint8_t minor,
                                       const std::vector<uint8_t> &payload = {});

  // BLE operations
  void write_data_(const std::vector<uint8_t> &data);
  void subscribe_notifications_();

  // State machine
  void advance_state_();
  void handle_notification_(const uint8_t *data, uint16_t length);
  void process_response_();
  void send_next_command_();

  // Sensor helpers
  const char *state_to_string_(uint8_t state);
  const char *activity_to_string_(uint8_t activity);

  // Config
  uint16_t pin_{0};

  // Protocol state
  ConnectionState state_{ConnectionState::IDLE};
  uint32_t channel_id_{0};
  uint16_t write_handle_{0};
  uint16_t notify_handle_{0};

  // Response accumulation
  std::vector<uint8_t> response_buffer_;

  // Command queue
  std::queue<CommandType> command_queue_;
  CommandType pending_command_{CommandType::NONE};
  uint32_t command_sent_at_{0};

  // Keepalive
  uint32_t last_keepalive_{0};

  // Timeout
  static constexpr uint32_t RESPONSE_TIMEOUT_MS = 30000;
  static constexpr uint32_t KEEPALIVE_INTERVAL_MS = 15000;

  // Sensors
#ifdef USE_SENSOR
  sensor::Sensor *battery_sensor_{nullptr};
#endif
#ifdef USE_TEXT_SENSOR
  text_sensor::TextSensor *state_sensor_{nullptr};
  text_sensor::TextSensor *activity_sensor_{nullptr};
  text_sensor::TextSensor *error_sensor_{nullptr};
#endif
#ifdef USE_BINARY_SENSOR
  binary_sensor::BinarySensor *charging_sensor_{nullptr};
  binary_sensor::BinarySensor *connected_sensor_{nullptr};
#endif
};

}  // namespace automower_ble
}  // namespace esphome
